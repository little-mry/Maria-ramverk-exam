...
const Cart = () => {
  ...
  return (
    <>
      <section className={styles.icon__con} onClick={() => setIsCartOpen(!isCartOpen)}>
            ...
            {cartItems.length > 0 && (
              <div>
                <article className={styles.total}>
                  ...
                  <p className={styles.total__price}>{cartItems.reduce((acc, item) => acc + item.price * item.quantity, 0)} SEK</p>
                </article>
                <button
